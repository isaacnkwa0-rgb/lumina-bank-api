...
export class AdminController {
  ...
  async sendBulkEmail(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const validTypes = ['ALL', 'SINGLE', 'SELECTED', 'KYC_PENDING', 'KYC_VERIFIED', 'KYC_REJECTED', 'TIER_STANDARD', 'TIER_PREMIUM', 'TIER_ELITE', 'TIER_PRIVATE', 'TIER_BUSINESS', 'MARKETING_CONSENT', 'SUSPENDED'];
  ...
  async getAdminNotification(req: Request, res: Response, next: NextFunction) {
    try {
      const data = await adminService.getAdminNotification(req.params.id as string);
      sendSuccess(res, data);
    } catch (err) { next(err); }
  }
